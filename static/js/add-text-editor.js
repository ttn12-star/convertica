/**
 * Add Text to PDF Editor
 *
 * PDF.js preview where the user clicks to place text boxes, whiteout
 * rectangles, and highlights; drags to position, pulls the corner handle
 * to resize, edits text inline (contenteditable). Object coordinates are
 * kept in PDF points so zooming never mutates the document model; on
 * submit the operations array is sent to /api/pdf-edit/add-text/.
 *
 * Extras over the sign editor it derives from: undo/redo, zoom,
 * per-object styling toolbar, background-sampling whiteout, and a
 * sessionStorage draft so a reload does not lose work.
 *
 * Reuses window helpers: showLoading, hideLoading, showError,
 * showDownloadButton, CSRF_TOKEN, API_URL, FILE_INPUT_NAME.
 */
document.addEventListener('DOMContentLoaded', () => {
    ...
    let mode = 'text';         // 'text' | 'whiteout' | 'highlight'
    let selectedId = null;
    let nextOpId = 1;
    let draftKey = null;
    /** ops: array of {id, type, page(1-idx), x, y, w, h (PDF points),
     *  text, fontKey, fontSize(pt), color, bold, italic, underline, align,
     *  element (DOM overlay, current page only)} */
    let ops = [];
    const undoStack = [];
    const redoStack = [];
    ...
    modeButtons.forEach((btn) => {
        btn.addEventListener('click', () => setMode(btn.dataset.editMode));
    });
    setMode('text');
    ...
    async function handleFileSelected(file) {
        ...
        if (!ok) {
            window.showError && window.showError(
                window.FAILED_TO_LOAD_PDF || 'Failed to load PDF preview.', 'editorResult'
            );
            return;
        }
        cleanupPreviousPDF();
        ...
        currentPage = 1;
        draftKey = 'convertica_addtext_' + file.name + '_' + file.size;
        tryRestoreDraft();
    ...
    function buildOverlayElement(op) {
        ...
        del.setAttribute('aria-label', window.DELETE_OBJECT_LABEL || 'Delete');
        del.textContent = '×';
    ...
    form.addEventListener('submit', async (ev) => {
        ...
        if (!selectedFile) {
            window.showError && window.showError(
                window.SELECT_FILE_MESSAGE || 'Please select a PDF first.', 'editorResult'
            );
            return;
        }
        ...
        if (payload.length === 0) {
            window.showError && window.showError(
                window.NO_OBJECTS_MESSAGE || 'Click on the page to add text first.', 'editorResult'
            );
            return;
        }
        ...
        try {
            ...
            const response = await fetch(window.API_URL, init);
            if (!response.ok) {
            ...
            submitButton && (submitButton.disabled = false);
            window.showError && window.showError(
                err.message || (window.ERROR_MESSAGE || 'Editing failed.'), 'editorResult'
            );
        }
    });
});
