/**
 * Excel to PDF converter JavaScript functionality.
 * Handles file upload, conversion, and progress tracking.
 */

class ExcelToPDFConverter {
    constructor() {
        this.form = document.getElementById('excelToPdfForm');
    ...
    enableBatchMode() {
        ...
        this.fileInput.accept = '.xls,.xlsx,application/vnd.ms-excel,application/vnd.openxmlformats-officedocument.spreadsheetml.sheet';

        // Update UI text to be more explicit about multi-file selection
        const dropZoneText = this.dropZone.querySelector('p.text-lg');
        if (dropZoneText) {
            dropZoneText.innerHTML = '<strong>Select multiple Excel files</strong> (up to 10 files)';
        }

        const dropZoneSubtext = this.dropZone.querySelector('p.text-sm');
        if (dropZoneSubtext) {
            dropZoneSubtext.textContent = 'Drag & drop or click to browse • .xls and .xlsx files up to 200MB each';
        }
        ...
        if (this.browseBtn) {
            ...
            if (btnText) {
                btnText.textContent = 'Select Excel files (multiple)';
            }
        }
    }
    ...
    async handleSubmit(e) {
        ...
        if (this.isBatchMode) {
            ...
            files.forEach((file, index) => {
                formData.append(`excel_files[${index}]`, file);
            });
            ...
            formData.append('excel_file', this.currentFile);
    ...
    showResult(result) {
        ...
        if (result.download_url) {
            ...
            this.downloadLink.download = this.currentFile.name.replace(/\.(xls|xlsx)$/i, '.pdf');
        }
    }
    ...
    resetProgress() {
        ...
        this.progressMessage.textContent = 'Please wait while we convert your Excel file to PDF...';
    }
    ...
    resetForm() {
        ...
        if (this.isBatchMode) {
            ...
            // Reset UI text
            const dropZoneText = this.dropZone.querySelector('p.text-lg');
            dropZoneText.textContent = 'Drop your Excel file here or click to browse';

            const dropZoneSubtext = this.dropZone.querySelector('p.text-sm');
            dropZoneSubtext.textContent = 'Support for .xls and .xlsx files up to 50MB';
        }
    }
}
