...
def index_page(request):
    ...
    if latest_articles is None:
        latest_articles = list(Article.objects.filter(
            status='published'
        ).select_related('category').order_by('-published_at', '-created_at')[:2])
...
def about_page(request):
    ...
    context = {
        ...
        'page_description': _('Learn about Convertica - your trusted online PDF tools platform. We are committed to constant improvement and providing the best PDF conversion and editing experience.'),
        'page_keywords': _('about Convertica, PDF tools company, online PDF converter, PDF services, document conversion'),
    }
...
def privacy_page(request):
    ...
    context = {
        ...
        'page_description': _('Read Convertica\'s Privacy Policy. Learn how we protect your data and handle your files. Your privacy is our priority.'),
        'page_keywords': _('privacy policy, data protection, file security, privacy, Convertica privacy'),
    }
...
def terms_page(request):
    ...
    context = {
        ...
        'page_description': _('Read Convertica\'s Terms of Service. Understand the terms and conditions for using our PDF tools and services.'),
        'page_keywords': _('terms of service, terms and conditions, user agreement, Convertica terms'),
    }
...
def contact_page(request):
    ...
    context = {
        ...
        'page_description': _('Contact Convertica for support, questions, or feedback. We\'re here to help with all your PDF tool needs.'),
...
def sitemap_xml(request):
    ...
    pages = [
        ...
        {'url': 'pdf-edit/add-page-numbers/', 'priority': '0.7', 'changefreq': 'monthly'},
        ...
        {'url': 'pdf-organize/remove-pages/', 'priority': '0.7', 'changefreq': 'monthly'},
        {'url': 'pdf-organize/extract-pages/', 'priority': '0.7', 'changefreq': 'monthly'},
    ...
    # Add blog articles - optimize with only() to load only needed fields
    published_articles = Article.objects.filter(
        status='published'
    ).only('slug', 'updated_at').order_by('-published_at')
    for article in published_articles:
        article_url = article.get_absolute_url()
        lastmod = article.updated_at.strftime('%Y-%m-%d') if article.updated_at else current_date
        sitemap += '  <url>\n'
        sitemap += f'    <loc>{base_url}{article_url}</loc>\n'
