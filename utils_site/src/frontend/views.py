...
def _get_related_tools(current_tool):
    ...
    all_tools = {
        ...
        # Favicon / ICO cluster. NOTE: the favicon generator's config key (and
        # thus its current_tool / relations key) is "generate_favicon", while
        # its route name is "favicon_generator_page".
        "generate_favicon": {
            "name": _("Favicon Generator"),
            "url": "frontend:favicon_generator_page",
            "description": _("Create a complete favicon package from any image"),
        ...
        "png_to_ico": {
            "name": _("PNG to ICO"),
            "url": "frontend:png_to_ico_page",
            "description": _("Convert PNG images to .ico favicons"),
            "icon": '<path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M4 16l4.586-4.586a2 2 0 012.828 0L16 16m-2-2l1.586-1.586a2 2 0 012.828 0L20 14m-6-6h.01M6 20h12a2 2 0 002-2V6a2 2 0 00-2-2H6a2 2 0 00-2 2v12a2 2 0 002 2z"/>',
            "gradient": "from-amber-500 to-orange-600",
        },
        "jpg_to_ico": {
            "name": _("JPG to ICO"),
            "url": "frontend:jpg_to_ico_page",
            "description": _("Convert JPG images to .ico favicons"),
        ...
        "svg_to_ico": {
            "name": _("SVG to ICO"),
            "url": "frontend:svg_to_ico_page",
            "description": _("Convert SVG vectors to .ico favicons"),
            "icon": '<path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M10 20l4-16m4 4l4 4-4 4M6 16l-4-4 4-4"/>',
            "gradient": "from-amber-500 to-orange-600",
        },
        "webp_to_ico": {
            "name": _("WebP to ICO"),
            "url": "frontend:webp_to_ico_page",
            "description": _("Convert WebP images to .ico favicons"),
        ...
        "ico_to_png": {
            "name": _("ICO to PNG"),
            "url": "frontend:ico_to_png_page",
            "description": _("Extract a PNG from an .ico file"),
    ...
    relations = {
        ...
        # Favicon / ICO cluster. The favicon generator's relations key is
        # "generate_favicon" (its config key), not "favicon_generator".
        "generate_favicon": ["png_to_ico", "convert_image", "optimize_image"],
        "png_to_ico": ["generate_favicon", "jpg_to_ico", "convert_image"],
        "jpg_to_ico": ["generate_favicon", "png_to_ico", "convert_image"],
...
#: tool_key -> (webp_rel, jpg_rel) static paths of the styled screenshot, or
...
def _tool_screenshot_paths(tool_key: str) -> tuple[str, str] | None:
    """Static rel paths of the tool's screenshot (generated by
    scripts/gen_tool_screenshots.py under static/images/tools/)."""
    if tool_key in _TOOL_SCREENSHOT_CACHE:
        return _TOOL_SCREENSHOT_CACHE[tool_key]
    from django.contrib.staticfiles import finders
    from django.urls import NoReverseMatch, reverse

    result = None
    try:
        url_name = _TOOL_URL_NAME_OVERRIDES.get(tool_key, f"{tool_key}_page")
        path = reverse(f"frontend:{url_name}")
        slug = path.strip("/").split("/", 1)[-1].replace("/", "-")
        webp_rel = f"images/tools/{slug}.webp"
        jpg_rel = f"images/tools/{slug}.jpg"
        if finders.find(webp_rel) and finders.find(jpg_rel):
            result = (webp_rel, jpg_rel)
    except NoReverseMatch:
        result = None
    _TOOL_SCREENSHOT_CACHE[tool_key] = result
    return result
...
def _render_tool_page(request, tool_key: str) -> HttpResponse:
    ...
    shots = _tool_screenshot_paths(tool_key)
    if shots:
        context["tool_screenshot"], context["tool_screenshot_jpg"] = shots
        # feeds og:image + the WebApplication JSON-LD image in base.html
        ...
        **(
            {
                ...
                "tool_screenshot": _shots[0],
                "tool_screenshot_jpg": _shots[1],
            }
            if _shots
            else {}
        ),
...
def workbench_page(request):
    ...
    context = {
        ...
        "workbench_i18n": {
            ...
            "boards": _("Boards"),
            "newBoard": _("New board…"),
            "createBoard": _("Create board"),
            "boardNamePlaceholder": _("Board name"),
            "startEmpty": _("Start empty"),
            "rename": _("Rename"),
            "setDefault": _("Set as default"),
            "defaultBadge": _("Default"),
            "duplicate": _("Duplicate"),
            "deleteBoard": _("Delete board"),
            "deleteConfirm": _("Delete this board and its %(count)s tiles?"),
            "lastBoard": _("You need at least one board."),
            "boardLimit": _("Board limit reached for your plan."),
            "cancel": _("Cancel"),
...
def _get_sitemap_pages():
    ...
    return [
        ...
        {"url": "image/favicon-generator/", "priority": "0.8", "changefreq": "monthly"},
        {"url": "image/png-to-ico/", "priority": "0.7", "changefreq": "monthly"},
        {"url": "image/jpg-to-ico/", "priority": "0.7", "changefreq": "monthly"},
