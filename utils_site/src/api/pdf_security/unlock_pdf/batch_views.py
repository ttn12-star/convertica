...
All files are unlocked with the same password.
"""
...
class UnlockPDFBatchAPIView(APIView):
    ...
    def post(self, request: HttpRequest):
        """Handle batch PDF unlock with ZIP output."""
        ...
        try:
            ...
            log_conversion_start(logger, "UNLOCK_PDF_BATCH", context)

            tmp_dir = tempfile.mkdtemp(prefix="unlock_batch_")
            output_files = []
            ...
            zip_path = os.path.join(tmp_dir, "unlocked_pdfs.zip")
            with zipfile.ZipFile(zip_path, "w", zipfile.ZIP_DEFLATED) as zipf:
                for original_name, output_path in output_files:
                    base_name = os.path.splitext(original_name)[0]
                    zip_name = f"{base_name}_unlocked.pdf"
