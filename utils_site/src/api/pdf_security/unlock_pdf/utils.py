...
def unlock_pdf(
    ...
    try:
        ...
        try:
            ...
            logger.debug("Unlock completed", extra={**context, "event": "unlock_complete"})
            
        except EncryptedPDFError:
            raise
        except Exception as e:
            error_context = {**context, "error_type": type(e).__name__, "error_message": str(e)}
            logger.error("PDF unlock failed", extra={**error_context, "event": "unlock_error"}, exc_info=True)
            raise ConversionError(f"Failed to unlock PDF: {e}", context=error_context) from e
        ...
        logger.info("PDF unlock successful", extra={
            **context, "event": "unlock_success",
