...
def rotate_pdf(
    ...
    try:
        ...
        try:
            logger.debug(
                "Writing PDF file to temporary location",
        ...
        try:
            logger.info(
                "Starting PDF rotation", extra={**context, "event": "rotation_start"}
            )
            ...
            logger.debug(
                "Rotation completed", extra={**context, "event": "rotation_complete"}
            )

        except Exception as rot_exc:
            error_context = {
                **context,
                "error_type": type(rot_exc).__name__,
                "error_message": str(rot_exc),
            }

            if (
                "encrypted" in str(rot_exc).lower()
                or "password" in str(rot_exc).lower()
            ):
                logger.warning(
            ...
            raise ConversionError(
                f"Rotation failed: {rot_exc}", context=error_context
            ) from rot_exc
