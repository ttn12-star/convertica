...
def get_page_position(position: str, page_width: float, page_height: float, font_size: int):
...
def add_page_numbers(
    ...
    try:
        ...
        # Add page numbers
        try:
            logger.info("Adding page numbers", extra={**context, "event": "add_numbers_start"})
            ...
            logger.debug("Page numbers added", extra={**context, "event": "add_numbers_complete"})
            
        except Exception as e:
            error_context = {**context, "error_type": type(e).__name__, "error_message": str(e)}
            logger.error("Failed to add page numbers", extra={**error_context, "event": "add_numbers_error"}, exc_info=True)
            raise ConversionError(f"Failed to add page numbers: {e}", context=error_context) from e
        ...
        logger.info("Page numbers added successfully", extra={
            **context, "event": "add_numbers_success",
