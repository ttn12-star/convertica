...
class ExtractPagesBatchAPIView(APIView):
    ...
    def post(self, request: HttpRequest):
        """Handle batch PDF page extraction with ZIP output."""
        ...
        try:
            ...
            log_conversion_start(logger, "EXTRACT_PAGES_BATCH", context)

            tmp_dir = tempfile.mkdtemp(prefix="extract_batch_")
            output_files = []

            for idx, pdf_file in enumerate(pdf_files):
                try:
                    pdf_path, extracted_path = extract_pages(
                        pdf_file,
                        pages=pages,
                        suffix="_convertica",
                    )
                    tmp_dirs_to_cleanup.add(os.path.dirname(pdf_path))
                    output_files.append((pdf_file.name, extracted_path))
            ...
            zip_path = os.path.join(tmp_dir, "extracted_pages.zip")
            with zipfile.ZipFile(zip_path, "w", zipfile.ZIP_DEFLATED) as zipf:
                for original_name, extracted_path in output_files:
                    base_name = os.path.splitext(original_name)[0]
                    zip_name = f"{base_name}_extracted.pdf"
                    zipf.write(extracted_path, zip_name)
