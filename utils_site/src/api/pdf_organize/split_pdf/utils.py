    ...
    try:
        ...
        try:
            ...
            if split_type == "page":
                ...
                if not pages:
                    raise ConversionError("Pages parameter required for range split", context=context)
            ...
            logger.debug("Split completed", extra={**context, "event": "split_complete", "num_files": len(output_files)})
            
        except Exception as split_exc:
            error_context = {**context, "error_type": type(split_exc).__name__, "error_message": str(split_exc)}
            logger.error("PDF split failed", extra={**error_context, "event": "split_error"}, exc_info=True)
            raise ConversionError(f"Failed to split PDF: {split_exc}", context=error_context) from split_exc
