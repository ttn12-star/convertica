...
class CompressPDFBatchAPIView(APIView):
    """Handle batch PDF compression requests."""

    @combined_rate_limit(group="api_batch", ip_rate="10/h", methods=["POST"])
    @compress_pdf_docs()
    def post(self, request: HttpRequest):
        """Handle batch PDF compression with ZIP output."""
        ...
        try:
            ...
            log_conversion_start(logger, "COMPRESS_PDF_BATCH", context)

            tmp_dir = tempfile.mkdtemp(prefix="compress_batch_")
            output_files = []

            for idx, pdf_file in enumerate(pdf_files):
                try:
                    pdf_path, compressed_path = compress_pdf(
                        pdf_file,
                        compression_level=compression_level,
                        suffix="_convertica",
                    )
                    tmp_dirs_to_cleanup.add(os.path.dirname(pdf_path))
                    output_files.append((pdf_file.name, compressed_path))
            ...
            zip_path = os.path.join(tmp_dir, "compressed_pdfs.zip")
            with zipfile.ZipFile(zip_path, "w", zipfile.ZIP_DEFLATED) as zipf:
                for original_name, compressed_path in output_files:
                    base_name = os.path.splitext(original_name)[0]
                    zip_name = f"{base_name}_compressed.pdf"
                    zipf.write(compressed_path, zip_name)
