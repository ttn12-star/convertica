...
class RemovePagesBatchAPIView(APIView):
    ...
    def post(self, request: HttpRequest):
        """Handle batch PDF page removal with ZIP output."""
        ...
        try:
            ...
            log_conversion_start(logger, "REMOVE_PAGES_BATCH", context)

            tmp_dir = tempfile.mkdtemp(prefix="remove_batch_")
            output_files = []
            ...
            zip_path = os.path.join(tmp_dir, "removed_pages.zip")
            with zipfile.ZipFile(zip_path, "w", zipfile.ZIP_DEFLATED) as zipf:
                for original_name, output_path in output_files:
                    base_name = os.path.splitext(original_name)[0]
                    zip_name = f"{base_name}_removed.pdf"
                    zipf.write(output_path, zip_name)

            response = FileResponse(
                open(zip_path, "rb"),
                as_attachment=True,
                filename="removed_pages.zip",
            )
