...
def remove_pages(
    ...
    try:
        ...
        # Remove pages
        try:
            logger.info("Removing pages from PDF", extra={**context, "event": "remove_pages_start"})
            ...
            logger.debug("Pages removed", extra={**context, "event": "remove_pages_complete"})
            
        except Exception as e:
            error_context = {**context, "error_type": type(e).__name__, "error_message": str(e)}
            logger.error("Failed to remove pages", extra={**error_context, "event": "remove_pages_error"}, exc_info=True)
            raise ConversionError(f"Failed to remove pages: {e}", context=error_context) from e
        ...
        logger.info("Pages removed successfully", extra={
            **context, "event": "remove_pages_success",
