    ...
    def validate_page_order(self, value):
        ...
        try:
            ...
            if not all(isinstance(x, int) and x >= 0 for x in order):
                raise serializers.ValidationError("page_order must contain non-negative integers.")
            # Check for duplicates
            if len(set(order)) != len(order):
                raise serializers.ValidationError("page_order contains duplicate page indices.")
            return order
