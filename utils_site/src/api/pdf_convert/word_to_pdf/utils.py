...
def convert_word_to_pdf(uploaded_file: UploadedFile, suffix: str = "_convertica") -> Tuple[str, str]:
    ...
    safe_name = sanitize_filename(get_valid_filename(os.path.basename(original_filename)))
    ...
    try:
        ...
        try:
            logger.debug("Writing Word file to temporary location", extra={**context, "event": "file_write_start"})
        ...
        if not is_valid:
            ...
            raise InvalidPDFError(
                validation_error or "Invalid Word file structure",
                context=context
            )

        # Perform LibreOffice conversion with improved settings
        logger.info("Starting LibreOffice conversion", extra={**context, "event": "conversion_start"})
        try:
            ...
            if "password" in error_msg or "protected" in error_msg:
                logger.warning(
                    ...
                    extra={**context, "event": "protected_file_error", "stderr": stderr[:500]}
                )
                raise ConversionError(
            ...
            raise ConversionError(
                ...
                context={**context, "error_type": "CalledProcessError", "return_code": e.returncode}
            ) from e
