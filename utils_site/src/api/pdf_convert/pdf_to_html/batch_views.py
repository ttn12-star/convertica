...
class PDFToHTMLBatchAPIView(BaseBatchAPIView):
    """Handle batch PDF → HTML conversion requests."""

    CONVERSION_TYPE = "PDF_TO_HTML_BATCH"
    TMP_PREFIX = "pdf2html_batch_"
    OUTPUT_ZIP_FILENAME = "pdf_to_html_convertica.zip"

    def convert_single(self, uploaded_file, context, **params):
        input_path, output_path = convert_pdf_to_html(
    ...
    def get_zip_entry_name(self, original_name, output_path):
        return f"{os.path.splitext(original_name)[0]}_convertica.html"

    @combined_rate_limit(group="api_batch", ip_rate="10/h", methods=["POST"])
    @batch_premium_docs(summary="Pdf To Html (batch, premium)", file_field="pdf_files")
