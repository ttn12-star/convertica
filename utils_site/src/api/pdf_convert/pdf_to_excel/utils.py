...
def convert_pdf_to_excel(
    ...
    try:
        ...
        try:
            logger.info("Starting PDF to Excel conversion", extra={**context, "event": "conversion_start"})
                ...
                for part in pages.split(','):
                    ...
                    if '-' in part:
                        ...
                        try:
                            ...
                            logger.warning(f"Invalid page number: {part}", extra=context)
                            continue
                ...
                with pdfplumber.open(pdf_path) as pdf:
                    ...
                    for page_num in pages_to_process:
                        if page_num < 0 or page_num >= total_pages:
                            logger.warning(f"Skipping invalid page number: {page_num + 1}", extra={**context, "page_num": page_num + 1})
                            continue
                        ...
                        try:
                            ...
                            try:
                                ...
                                logger.debug(f"Default table extraction failed for page {page_num + 1}: {e}", extra={**context, "page": page_num + 1})
                            ...
                            if not tables or len(tables) == 0:
                                try:
                                    ...
                                    logger.debug(f"Aggressive table extraction failed for page {page_num + 1}: {e}", extra={**context, "page": page_num + 1})
                            ...
                            if not tables or len(tables) == 0:
                                try:
                                    ...
                                    logger.debug(f"Text-based table extraction failed for page {page_num + 1}: {e}", extra={**context, "page": page_num + 1})
                            
                            page_has_valid_tables = False
                            
                            if tables and len(tables) > 0:
                                logger.debug(f"Found {len(tables)} table(s) on page {page_num + 1}", extra={**context, "page": page_num + 1, "tables_count": len(tables)})
                                ...
                                for table in tables:
                                    ...
                                    if has_data and non_empty_cells >= 2:  # At least 2 non-empty cells
                                        all_tables.append({
                                            'page': page_num + 1,
                                            'table': table
                                        })
                                        page_has_valid_tables = True
                                        logger.debug(f"Valid table found on page {page_num + 1} with {non_empty_cells} non-empty cells", extra={**context, "page": page_num + 1, "cells": non_empty_cells})
                                    else:
                                        logger.debug(f"Empty or insufficient table found on page {page_num + 1} (only {non_empty_cells} non-empty cells)", extra={**context, "page": page_num + 1, "cells": non_empty_cells})
                            else:
                                logger.debug(f"No tables found on page {page_num + 1}", extra={**context, "page": page_num + 1})
                            ...
                            logger.warning(
                                ...
                                extra={**context, "page": page_num + 1, "error": str(page_err)},
                                exc_info=True
                            )
                ...
                if page_numbers:
                    ...
                    try:
                        ...
                        raise ConversionError(
                            f"Failed to process PDF: {pdf_err}",
                            context=context
                        ) from pdf_err
                ...
                if all_tables:
                    for idx, table_data in enumerate(all_tables):
                        ...
                        try:
                            ...
                            if headers:
                                headers = [str(h).strip() if h is not None else f"Column {i+1}" for i, h in enumerate(headers)]
                            ...
                            if df.empty:
                                logger.warning(f"Table on page {page_num} is empty after cleaning", extra={**context, "page": page_num})
                                continue
                            ...
                            counter = 1
                            while sheet_name in [ws.title for ws in writer.book.worksheets if hasattr(writer, 'book')]:
                                sheet_name = f"{base_sheet_name[:28]}_{counter}"
                                counter += 1
                            
                            df.to_excel(writer, sheet_name=sheet_name, index=False)
                            logger.debug(f"Added table from page {page_num} to Excel", extra={**context, "page": page_num, "rows": len(df)})
                            
                        except Exception as table_err:
                            logger.warning(
                                f"Failed to add table from page {page_num}: {table_err}",
                                extra={**context, "page": page_num, "error": str(table_err)},
                                exc_info=True
                            )
                ...
                if pages_with_images:
                    ...
                    try:
                        logger.info(
                            ...
                            extra={**context, "pages_count": len(pages_with_images), "pages": pages_with_images}
                        )
                        ...
                        for page_num in pages_with_images:
                            ...
                            try:
                                ...
                                img_path = os.path.join(tmp_dir, f"page_{page_num}_image.jpg")
                                ...
                                worksheet.column_dimensions['A'].width = min(img_obj.width / 7, 100)  # Excel column width units
                                
                                logger.debug(f"Added image from page {page_num} to Excel", extra={**context, "page": page_num})
                                
                            except Exception as img_err:
                                logger.warning(
                                    f"Failed to add image from page {page_num}: {img_err}",
                                    extra={**context, "page": page_num, "error": str(img_err)},
                                    exc_info=True
                                )
                                continue
                        ...
                        if not all_tables:
                            try:
                                note_sheet = workbook.create_sheet(title="Note", index=0)
                                note_sheet['A1'] = "No tables found in PDF"
                                note_sheet['A2'] = "Pages have been converted to images instead."
                                note_sheet['A3'] = "Please check the image sheets for page content."
                            except Exception:
                                pass
            ...
            if "pdfplumber" in missing_module or "pdfplumber" in str(import_err):
                ...
                raise ConversionError(
                    f"Required module not available: {missing_module}",
                    context=context
                ) from import_err
        except Exception as conv_exc:
            error_context = {**context, "error_type": type(conv_exc).__name__, "error_message": str(conv_exc)}
            logger.error("PDF to Excel conversion failed", extra={**error_context, "event": "conversion_error"}, exc_info=True)
            raise ConversionError(f"Failed to convert PDF to Excel: {conv_exc}", context=error_context) from conv_exc
        
        # Validate output
        is_valid, validation_error = validate_output_file(output_path, min_size=1000, context=context)
        if not is_valid:
            raise ConversionError(validation_error or "Output Excel file is invalid", context=context)
        
        output_size = os.path.getsize(output_path)
        logger.info("PDF to Excel conversion successful", extra={
            **context, "event": "conversion_success",
