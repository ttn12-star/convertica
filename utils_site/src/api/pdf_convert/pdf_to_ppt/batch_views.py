...
class PDFToPowerPointBatchAPIView(APIView):
    ...
    def post(self, request: HttpRequest):
        """Handle batch PDF to PowerPoint conversion with ZIP output."""
        ...
        try:
            ...
            log_conversion_start(logger, "PDF_TO_PPT_BATCH", context)

            tmp_dir = tempfile.mkdtemp(prefix="pdf2ppt_batch_")
            output_files = []

            for idx, pdf_file in enumerate(pdf_files):
                try:
                    pdf_path, ppt_path = convert_pdf_to_ppt(
                        pdf_file,
                        suffix="_convertica",
                    )
                    tmp_dirs_to_cleanup.add(os.path.dirname(pdf_path))
                    output_files.append((pdf_file.name, ppt_path))
            ...
            zip_path = os.path.join(tmp_dir, "pdf_to_ppt_batch.zip")
            with zipfile.ZipFile(zip_path, "w", zipfile.ZIP_DEFLATED) as zipf:
                for original_name, ppt_path in output_files:
                    base_name = os.path.splitext(original_name)[0]
                    zip_name = f"{base_name}_convertica.pptx"
                    zipf.write(ppt_path, zip_name)

            response = FileResponse(
                open(zip_path, "rb"),
                as_attachment=True,
                filename="pdf_to_ppt_convertica.zip",
            )
