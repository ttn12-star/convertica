...
from src.exceptions import ConversionError, StorageError, InvalidPDFError
...
def convert_jpg_to_pdf(
    ...
    try:
        ...
        try:
            logger.info("Starting JPG to PDF conversion", extra={**context, "event": "conversion_start"})
            ...
            try:
                ...
                logger.error(
                    "Invalid image file",
                    extra={**context, "error": str(img_err), "event": "invalid_image_error"},
                    exc_info=True
                )
                raise InvalidPDFError(
                    f"Invalid or corrupted image file: {img_err}",
                    context=context
                ) from img_err
            ...
            if image.mode in ('RGBA', 'LA', 'P'):
                logger.debug("Converting image to RGB", extra={**context, "original_mode": image.mode})
            ...
            logger.error(
                ...
                extra={**context, "error": str(img_err), "event": "image_processing_error"},
                exc_info=True
            )
        ...
        try:
            ...
            logger.debug("PDF created successfully", extra={**context, "event": "pdf_created"})
        ...
        is_valid, validation_error = validate_output_file(pdf_path, min_size=500, context=context)
        if not is_valid:
            logger.error(
        ...
        for idx, uploaded_file in enumerate(uploaded_files):
            ...
            try:
                logger.debug(
                    ...
                    extra={**context, "event": "file_write_start", "file_index": idx + 1, "filename": safe_name}
                )
                with open(image_path, "wb") as f:
                    for chunk in uploaded_file.chunks():
                        f.write(chunk)
                ...
                logger.debug(
                    ...
                    extra={**context, "event": "file_write_success", "file_index": idx + 1}
                )
            except (OSError, IOError) as io_err:
                logger.error(
                    f"Failed to write uploaded file {idx + 1}",
                    extra={**context, "event": "file_write_error", "error": str(io_err), "file_index": idx + 1},
                    exc_info=True
                )
                raise StorageError(
                    f"Failed to write uploaded file {idx + 1}: {io_err}",
                    context={**context, "error_type": type(io_err).__name__, "file_index": idx + 1}
                ) from io_err
                ...
                try:
                    ...
                    try:
                        ...
                        logger.error(
                            ...
                            extra={**context, "error": str(img_err), "event": "invalid_image_error", "file_index": idx + 1},
                            exc_info=True
                        )
                        raise InvalidPDFError(
                    ...
                    if image.mode in ('RGBA', 'LA', 'P'):
                        logger.debug(
                            ...
                            extra={**context, "original_mode": image.mode, "file_index": idx + 1}
                        )
                        rgb_image = Image.new('RGB', image.size, (255, 255, 255))
                        if image.mode == 'P':
                            image = image.convert('RGBA')
                    ...
                    logger.debug(
                        ...
                        extra={**context, "event": "image_added", "file_index": idx + 1}
                    )
                    ...
                    logger.error(
                        ...
                        extra={**context, "error": str(img_err), "event": "image_processing_error", "file_index": idx + 1},
                        exc_info=True
                    )
                    raise ConversionError(
                        f"Error processing image {idx + 1}: {img_err}",
                        context={**context, "error_type": type(img_err).__name__, "file_index": idx + 1}
                    ) from img_err
