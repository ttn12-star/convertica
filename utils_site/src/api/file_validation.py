...
def validate_pdf_file(file_path: str, context: dict) -> Tuple[bool, Optional[str]]:
    ...
    try:
        ...
        try:
            ...
            logger.debug("PyPDF2 not available, skipping deep PDF validation", extra=context)
        except Exception as e:
            logger.warning(
            ...
            if header.startswith(DOCX_MAGIC):
                ...
                try:
                    ...
                    logger.warning(
                        ...
                        extra={**context, "error": str(zip_err), "event": "docx_zip_error"}
                    )
                    ...
                    logger.warning(
                        ...
                        extra={**context, "error": str(e), "event": "docx_validation_warning"}
                    )
            ...
            # Check for old DOC format (OLE2) - \xd0\xcf\x11\xe0\xa1\xb1\x1a\xe1
                ...
                if is_docx:
                    ...
                    try:
                        ...
                        with zipfile.ZipFile(file_path, 'r') as zip_file:
                            file_list = zip_file.namelist()
                            if any('word/document.xml' in f or '[Content_Types]' in f for f in file_list):
                                logger.info(
                                    "DOCX validated by structure despite missing magic number",
                                    extra={**context, "event": "docx_structure_validation"}
                                )
                                return True, None
                    except Exception as zip_err:
                        logger.debug(
                            "Extension-based DOCX validation failed",
                            extra={**context, "error": str(zip_err), "event": "docx_extension_validation_failed"}
                        )
                ...
                return False, f"File does not appear to be a valid Word document (header: {header_hex[:16]})"
...
def check_disk_space(path: str, required_mb: int = 100) -> Tuple[bool, Optional[str]]:
    ...
    try:
        ...
        if free_mb < required_mb:
            return False, f"Insufficient disk space. Required: {required_mb} MB, Available: {free_mb:.1f} MB"
...
def validate_output_file(file_path: str, min_size: int = 100, context: dict = None) -> Tuple[bool, Optional[str]]:
    """
    ...
    try:
        ...
        if file_size < min_size:
            logger.warning(
                ...
                extra={**context, "file_path": file_path, "file_size": file_size, "event": "small_output_file"}
            )
        ...
        logger.error(
            ...
            extra={**context, "file_path": file_path, "error": str(e), "event": "output_validation_error"},
            exc_info=True
        )
