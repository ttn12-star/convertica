...
class ArticleCategoryAdmin(admin.ModelAdmin):
    ...
    fieldsets = (
        (_("Basic Information"), {"fields": ("name_en", "slug", "description_en")}),
        (
            _("Translations"),
            {
                "fields": ("translations",),
                "description": _(
                    "Add translations for other languages using the tabs above."
                ),
            },
        ),
        (
    ...
    def translations_preview(self, obj):
        ...
        available_langs = [
            lang for lang in languages if lang in [l[0] for l in settings.LANGUAGES]
        ]
...
class ArticleAdmin(admin.ModelAdmin):
    ...
    fieldsets = (
        ...
        (
            _("Translations"),
            {
                "fields": ("translations",),
                "description": _(
                    "Add translations for other languages using the tabs above. Each language has its own tab with all necessary fields."
                ),
            },
        ),
        (
            _("AI Metadata"),
            {
                ...
                "classes": ("collapse",),
                "description": _(
                    "Additional metadata for AI agents to better understand and index your content."
                ),
            },
        ),
    ...
    def translations_preview(self, obj):
        ...
        if not obj.translations:
            return format_html(
                '<span style="color: #999;">{}</span>', _("No translations")
            )
        ...
        if not available_langs:
            return format_html(
                '<span style="color: #999;">{}</span>', _("No valid translations")
            )
