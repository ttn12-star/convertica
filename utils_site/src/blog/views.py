...
def article_list(request):
    ...
    articles = Article.objects.filter(status='published').select_related('category').order_by('-published_at', '-created_at')
    ...
    if search_query:
        ...
        if cached_results is not None:
            ...
            if language_code != 'en' and language_code in ['ru', 'pl', 'hi', 'es', 'id']:
                # Get all articles (respecting category filter if any) - only load needed fields
                all_articles = Article.objects.filter(status='published').only(
                    'id', 'translations', 'published_at', 'created_at', 'category_id'
                ...
                for article in all_articles[:1000]:
                    ...
                    if article.translations and language_code in article.translations:
                        ...
                        if (trans.get('title', '').lower().find(search_lower) != -1 or
                            trans.get('content', '').lower().find(search_lower) != -1 or
                            trans.get('excerpt', '').lower().find(search_lower) != -1):
                            # Reload full article for results
                            full_article = Article.objects.select_related('category').get(id=article.id)
                ...
                articles_list.sort(key=lambda x: (x.published_at or x.created_at) if (x.published_at or x.created_at) else x.id, reverse=True)
...
def article_detail(request, slug):
    ...
    related_articles = Article.objects.filter(
        ...
        status='published'
    ).exclude(id=article.id).select_related('category').order_by('-published_at', '-created_at')[:3]
    
    # If not enough articles in same category, get latest articles
    if len(related_articles) < 3:
        additional = Article.objects.filter(
            status='published'
        ).exclude(id=article.id).exclude(id__in=[a.id for a in related_articles]).select_related('category').order_by('-published_at', '-created_at')[:3 - len(related_articles)]
