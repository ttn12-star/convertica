        ...
        for article in articles:
            ...
            for lang_code in ['ru', 'pl', 'hi', 'es', 'id']:
                if lang_code not in article.translations:
                    self.stdout.write(self.style.WARNING(f'   ⚠️  Missing translation for {lang_code}'))
                    issues_found += 1
                    continue
                ...
                if 'content' not in trans or not trans['content']:
                    self.stdout.write(self.style.ERROR(f'   ❌ {lang_code}: Content missing'))
                    issues_found += 1
                else:
                    ...
                    if '\n\n\n' in content or '  ' in content:
                        self.stdout.write(self.style.WARNING(f'   ⚠️  {lang_code}: Excessive whitespace found'))
                        issues_found += 1
                        
                        if fix:
                            cleaned = self.clean_html_content(content)
                            trans['content'] = cleaned
                            self.stdout.write(self.style.SUCCESS(f'   ✅ {lang_code}: Cleaned (was {original_length} chars, now {len(cleaned)} chars)'))
                            fixed_count += 1
            ...
            if article.content_en:
                ...
                if '\n\n\n' in article.content_en or '  ' in article.content_en:
                    self.stdout.write(self.style.WARNING('   ⚠️  English: Excessive whitespace found'))
                    issues_found += 1
                    
                    if fix:
                        article.content_en = self.clean_html_content(article.content_en)
                        self.stdout.write(self.style.SUCCESS(f'   ✅ English: Cleaned (was {original_length} chars, now {len(article.content_en)} chars)'))
                        fixed_count += 1
