...
class Article(models.Model):
    ...
    relevant_tool = models.CharField(
        ...
        choices=[
            ...
            ("resize_pdf", "Change PDF Page Size"),
    ...
    @property
    def cover_image_webp_url(self):
        """Static URL of the .webp sibling of ``cover_image``, or None.

        Only static-path covers get a WebP variant (generated by
        scripts/optimize_images.py and committed next to the JPG).
        """
        if not self.cover_image or "." not in self.cover_image:
            return None
        webp_rel = self.cover_image.rsplit(".", 1)[0] + ".webp"
        from django.contrib.staticfiles import finders

        if finders.find(webp_rel):
            from django.templatetags.static import static

            return static(webp_rel)
        return None
