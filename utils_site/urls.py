...
from src.payments.polar_webhook import polar_webhook
...
urlpatterns = [
    ...
    path("payments/webhook/polar/", polar_webhook, name="polar_webhook"),
    path(
...
urlpatterns += i18n_patterns(
    # Mirror /api/ landing under the locale prefix so /ru/api/, /es/api/ etc.
    # work and the menu links generated by {% url 'api_landing' %} resolve to
    # the currently-active locale. The bare /api/ route at the root level
    # still answers for SEO and direct hits.
    path("api/", api_landing, name="api_landing"),
